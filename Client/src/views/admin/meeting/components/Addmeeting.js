...
const AddMeeting = (props) => {
    ...
    const AddData = async (values, resetForm) => {
        setIsLoding(true);
        try {
            const res = await postApi('api/meeting/add', values); // Adjust endpoint if needed
            toast.success(res?.message || 'Meeting added!');
            resetForm();
            onClose();
            if (fetchData) fetchData(); // Refresh parent data if needed
        } catch (err) {
            toast.error(err?.response?.data?.error || 'Failed to add meeting');
        } finally {
            setIsLoding(false);
        }
    };
